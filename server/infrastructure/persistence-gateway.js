...
import pg from "pg";
import { Memo, Routine, TimeSession, Todo } from "../domain/entities.js";
import { TodoTree } from "../domain/todo-tree.js";
import { TodoWorkspace } from "../domain/todo-workspace.js";
import { ArchivePolicy } from "../domain/archive-policy.js";
import { TodoGateway } from "../application/ports.js";
...
const archivePolicy = new ArchivePolicy({ afterMonths: archiveAfterMonths });
...
function cleanRoutine(routine) {
  return Routine.from(routine).toJSON();
}

/** 오늘 요일에 해당하는 루틴을 오늘 할 일로 만든다. 이미 만든 건 건너뛴다. */
function materializeRoutines(data) {
  const { key: today, weekday } = todayInAppZone();
  const workspace = TodoWorkspace.from(data);
  const created = workspace.materializeRoutines({ today, weekday, idFactory: randomUUID });
  data.todos = workspace.toJSON().todos;
  return created;
}
...
function resetStaleRoutineTodos(data) {
  ...
  const workspace = TodoWorkspace.from(data);
  const staleIds = workspace.removeStaleRoutineTodos(today);
  data.todos = workspace.toJSON().todos;
  return staleIds;
}

function cleanTodo(todo) {
  return Todo.from(todo).toJSON();
}

function cleanMemo(memo) {
  return Memo.from(memo).toJSON();
}

function cleanSession(session) {
  return TimeSession.from(session).toJSON();
}

function cleanData(value) {
  return TodoWorkspace.from(value).toJSON();
}
...
function validateTodoTree(todos) {
  return TodoTree.validate(todos);
}
...
export class PersistenceGatewayAdapter extends TodoGateway {
  constructor() { super(); }
  async health() {
    let database = "none";
    if (pool) {
    ...
    return { ok: !database.startsWith("error"), storage: pool ? "postgres" : "file", database };
  }
  getData(userId) { return readDataWithRoutines(userId); }
  replaceData(value, userId) { return writeData(value, userId); }
  createMemo(value, userId) { return createMemoWithTodos(value, userId); }
  updateMemo(id, value, userId) { return updateMemo(id, value, userId); }
  deleteMemo(id, userId) { return deleteMemoById(id, userId); }
  reorderMemos(ids, userId) { return reorderMemosByIds(ids, userId); }
  createTodo(value, userId) { return createTodo(value, userId); }
  updateTodo(id, value, userId) { return updateTodo(id, value, userId); }
  deleteTodo(id, userId) { return deleteTodoById(id, userId); }
  reorderTodos(items, userId) { return reorderTodos(items, userId); }
  createSession(value, userId) { return createSession(value, userId); }
  deleteSession(id, userId) { return deleteSessionById(id, userId); }
  createRoutine(value, userId) { return createRoutine(value, userId); }
  updateRoutine(id, value, userId) { return updateRoutine(id, value, userId); }
  deleteRoutine(id, userId) { return deleteRoutineById(id, userId); }
}
export const runtimeConfig = { port, publicDir, allowedOrigins, maxBodyBytes, supabaseUrl, supabaseAnonKey };
