...
import { MemoScreen, ScheduleScreen, TimerScreen, TodoScreen } from "./src/Screens";
import { useAppData } from "./src/useAppData";

WebBrowser.maybeCompleteAuthSession();
type Workspace = "todo" | "memo" | "timer" | "schedule";
const tabs: { key: Workspace; label: string; icon: string }[] = [
  { key: "todo", label: "할 일", icon: "✓" }, { key: "memo", label: "메모", icon: "✎" },
  { key: "timer", label: "타이머", icon: "◷" }, { key: "schedule", label: "기록", icon: "▦" },
];
...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" />
    ...
    <View style={styles.content}>{store.loading ? <ActivityIndicator style={styles.loader} color="#176b47" /> : workspace === "todo" ? <TodoScreen store={store} /> : workspace === "memo" ? <MemoScreen store={store} /> : workspace === "timer" ? <TimerScreen store={store} /> : <ScheduleScreen store={store} />}</View>
    <View style={styles.nav}>{tabs.map((tab) => <Pressable key={tab.key} style={styles.navItem} onPress={() => setWorkspace(tab.key)}><Text style={[styles.navIcon, workspace === tab.key && styles.navActive]}>{tab.icon}</Text><Text style={[styles.navLabel, workspace === tab.key && styles.navActive]}>{tab.label}</Text></Pressable>)}</View>
  </SafeAreaView>;
}
...
const styles = StyleSheet.create({
  ...
  topbar: { minHeight: 68, flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#dfe5df", backgroundColor: "#fff" }, brand: { fontSize: 24, fontWeight: "800", color: "#173829" }, email: { marginTop: 1, fontSize: 11, color: "#7c8780" }, topActions: { flexDirection: "row", gap: 16 }, refresh: { fontSize: 12, fontWeight: "600", color: "#176b47" }, logout: { fontSize: 12, fontWeight: "600", color: "#7a5b56" }, errorBar: { alignItems: "center", padding: 8, backgroundColor: "#fff0ed" }, errorText: { fontSize: 12, color: "#a23c32" }, content: { flex: 1 }, loader: { flex: 1 },
