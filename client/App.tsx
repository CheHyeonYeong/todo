  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" />
    <View style={desktop ? styles.desktopShell : styles.mobileShell}>
      <View style={styles.mainColumn}>
        ...
        <View style={[styles.content, desktop && styles.desktopContent]}>{store.loading ? <ActivityIndicator style={styles.loader} color="#176b47" /> : desktop ? <View style={styles.dashboard}><View style={styles.dashboardPrimary}><View style={[styles.dashboardPanel, styles.todoPanel]}><TodoScreen store={store} /></View><View style={[styles.dashboardPanel, styles.memoPanel]}><MemoScreen store={store} /></View></View><View style={[styles.dashboardPanel, styles.timePanel]}><TimeScreen store={store} /></View></View> : workspace === "todo" ? <TodoScreen store={store} /> : workspace === "memo" ? <MemoScreen store={store} /> : <TimeScreen store={store} />}</View>
        {!desktop && navigation}
      </View>
    </View>
  </SafeAreaView>;
...
const styles = StyleSheet.create({
  ...
  dashboard: { flex: 1, flexDirection: "row", gap: 12, padding: 12, overflow: "hidden" }, dashboardPrimary: { flex: 1.65, minWidth: 0, gap: 12 }, dashboardPanel: { minHeight: 0, overflow: "hidden", borderWidth: 1, borderColor: "#dce3dd", borderRadius: 16, backgroundColor: "#fff" }, todoPanel: { flex: 1.2 }, memoPanel: { flex: 1 }, timePanel: { flex: 1, minWidth: 390 },
